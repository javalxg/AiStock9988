...
| 大涨事件研究的右侧跟随/延长持有结论（只迁移研究假设） | `src/aistock9988/backtest/engine.py` 的配置化 `time_exit_extension` | 单一 H10 盈利延长 H20 实验 | 禁止复制旧执行代码；延长判断只读计划退出日前一收盘，固定规则不扫描持有期或收益阈值，控制组必须同快照复现 |
