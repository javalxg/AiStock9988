...
| q70 收盘止损、下一可交易开盘退出语义（不迁移旧标签代码） | `src/aistock9988/labeling/executable_path.py` | 已实现，待全量历史审计 | 标签必须使用实际触发幅度和可成交退出价；买卖状态与唯一回测引擎一致；包含冻结成本；禁止把所有止损钳制为阈值收益 |
...
| 大涨事件研究的右侧跟随/延长持有结论（只迁移研究假设） | 单一 H10 盈利延长 H20 实验 | 已否决并从主线删除 | 复现实现在提交 `c44b57d`；组合收益因仓位占用下降，禁止复制旧代码、扫描持有期或保留被否决的引擎开关 |
| 旧 breadth-relative-turnover 实验的普通强势突破对照假设（不迁移广度排序或代码） | 2026 DB 全市场透明强势突破规则 | 已否决并从主线删除 | 2026 Base -6.71%、PF 0.744、MaxDD -17.49%；旧广度排序 -11.41% 也已关闭。复核证据保留在 `docs/council_20260828/STRONG_MOMENTUM_BREAKOUT_V1_2026_TO_DB_CUTOFF_20260902/`，禁止跨年救活、参数扫描或与 CAP1 组合 |
