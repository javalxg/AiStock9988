"""The two versioned, user-editable experiment configuration objects."""
...
class StrategyConfig:
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "StrategyConfig":
        ...
        if "identity" not in source:
            source = _upgrade_legacy_strategy(source)
        sections = {
...
class ModelConfig:
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "ModelConfig":
        source = _mapping(raw, "model config")
        if "identity" not in source:
            source = _upgrade_legacy_model(source)
        sections = {
            name: _mapping(source.get(name, {}), name)
            for name in ("identity", "scope", "features", "label", "walk_forward", "params", "prediction")
        }
        identity = sections["identity"]
        _identifier(identity.get("model_id"), "identity.model_id")
        if int(identity.get("version", 0)) <= 0:
            raise ValueError("identity.version must be positive")
        enabled = bool(identity.get("enabled", False))
        algorithm = str(identity.get("algorithm", "disabled"))
        if enabled and algorithm != "xgboost_ranker":
            raise ValueError("enabled model requires algorithm=xgboost_ranker")
        if not enabled and algorithm != "disabled":
            raise ValueError("disabled model requires algorithm=disabled")
        return cls(**{name: _freeze(value) for name, value in sections.items()})
    ...
    def model_id(self) -> str:
        return str(self.identity["model_id"])

    @property
    def enabled(self) -> bool:
        return bool(self.identity["enabled"])
    ...
    @property
    def feature_set_id(self) -> str:
        return str(self.identity.get("feature_set_id", "disabled.features"))

    @property
    def label_profile_id(self) -> str:
        return str(self.identity.get("label_profile_id", "disabled.label"))
...
def validate_strategy_model(strategy: StrategyConfig, model: ModelConfig) -> None:
    if strategy.mode == "rules" and model.enabled:
        raise ValueError("rules strategy requires a disabled model")
    if strategy.mode in {"xgboost", "hybrid"} and not model.enabled:
        raise ValueError(f"{strategy.mode} strategy requires an enabled model")
    if strategy.mode == "hybrid" and model.scope.get("training_population") != model.scope.get("prediction_population"):
        raise ValueError("hybrid training and prediction populations must match")


def _upgrade_legacy_strategy(source: Mapping[str, Any]) -> dict[str, Any]:
    candidate = _mapping(source.get("candidate", {}), "candidate")
    if candidate.get("filters"):
        raise ValueError("legacy natural-language strategy filters are prototypes and cannot run in V3")
    return {
        "identity": {"strategy_id": source.get("strategy_id"), "version": 1, "mode": source.get("strategy_type", "rules"),
                     "feature_set_id": source.get("feature_set_id")},
        "universe": source.get("universe", {}),
        "data_policy": source.get("data_policy", {}),
        "decision": source.get("decision", {"frequency": "daily", "entry_delay_sessions": 1}),
        "features": source.get("features", {}),
        "stage1": source.get("stage1", {}),
        "ranking": source.get("ranking", candidate.get("ranking", {})),
        "portfolio": source.get("portfolio", source.get("selection", {})),
        "execution": source.get("execution", {}),
        "acceptance": source.get("acceptance", {}),
    }


def _upgrade_legacy_model(source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "identity": {"model_id": source.get("model_id"), "version": 1, "enabled": source.get("enabled", False),
                     "algorithm": source.get("algorithm", "disabled"), "feature_set_id": source.get("feature_set_id"),
                     "label_profile_id": source.get("label_profile_id")},
        "scope": source.get("scope", source.get("prediction", {})),
        "features": source.get("features", {}),
        "label": source.get("label", {}),
        "walk_forward": source.get("walk_forward", source.get("training", {})),
        "params": source.get("params", {}),
        "prediction": source.get("prediction", {}),
    }


__all__ = ["StrategyConfig", "ModelConfig", "validate_strategy_model"]
