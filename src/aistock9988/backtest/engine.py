...
def run_backtest(signals: pd.DataFrame, prices: pd.DataFrame, *, config: BacktestConfig = BacktestConfig()) -> dict[str, pd.DataFrame]:
    ...
    for i, day in enumerate(sessions):
        ...
        for code, pos in list(positions.items()):
            ...
            ret = prior_close / pos["entry_price"] - 1.0
            if (pos["exit_due"] == day) or (config.stop_loss_pct is not None and ret <= config.stop_loss_pct) or (config.take_profit_pct is not None and ret >= config.take_profit_pct):
                row = by_key.loc[(day, code)] if (day, code) in by_key.index else None
                if row is not None:
                    execute(day, code, "SELL", float(row.raw_open), pos["shares"], "scheduled_or_rule_exit")
                    del positions[code]
        ...
        if previous in signal_map:
            ...
            for code in available.head(max(0, slots))["ts_code"]:
                ...
                positions[code] = {"shares": shares, "entry_price": price, "entry_date": day,
                                   "exit_due": sessions[exit_idx] if exit_idx < len(sessions) else None,
                                   "last_close": float(row.raw_close)}
    ...
    for code, pos in list(positions.items()):
        if (final_day, code) in by_key.index:
            ...
            execute(final_day, code, "SELL", float(row.raw_close), pos["shares"], "end_of_test_liquidation")
        del positions[code]
