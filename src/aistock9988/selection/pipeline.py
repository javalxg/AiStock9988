...
def build_rule_ledgers(
    ...
    rank_ready = ready & frame["stage1_pass"]
    ...
    frame["score_rejection_reason"] = np.select(
        [~frame["universe_pass"].astype(bool), ~frame["feature_ready"].astype(bool), ~frame["stage1_pass"]],
        ["UNIVERSE_REJECTED", frame["feature_rejection_reason"], "STAGE1_REJECTED"],
        default="",
    )
    frame["candidate_rank"] = pd.Series(pd.NA, index=frame.index, dtype="Int64")
    passed = frame[frame["stage1_pass"]].sort_values(
        ["asof", "final_score", "ts_code"], ascending=[True, False, True], kind="mergesort"
    )
    ...
    frame["candidate_status"] = np.select(
        [(~frame["stage1_pass"]).to_numpy(dtype=bool), in_view],
        ["REJECTED", "IN_VIEW"],
        default="BELOW_VIEW",
    )
