...
def compute_sector_relative(frame: pd.DataFrame, industry_map: Mapping[str, str],
    ...
    """Compute per-industry mean deviations for one signal-date cross-section.

    ``industry_map`` must already be resolved for the signal date. Missing
    memberships receive neutral zero values and are not used in means.
    """
    ...
    for col in cols:
        mean = values.groupby("_industry", sort=False)[col].transform("mean")
        output.loc[common, f"{col}_sector_rel"] = values[col] - mean
    return output
